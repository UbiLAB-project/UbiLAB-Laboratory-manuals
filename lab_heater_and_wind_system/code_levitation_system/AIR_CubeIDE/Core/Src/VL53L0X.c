...
#include "VL53L0X.h"
#include "I2CDev.h"
#include "stm32f7xx_hal.h" //SELECT proper MCU !!!!!!!! //OCTOBER 2019 AS
#include <stdbool.h>
#include <stdlib.h>
#include <stdint.h>
#include <string.h>

#define I2C &hi2c4  //SELECT proper I2C !!!!!!!! //OCTOBER 2019 AS

// SENSOR MODE
		
	 //#define HIGH_SPEED  //Time to read 1.3ms
	#define HIGH_ACCURACY  //Time to read 1.3ms
	//#define LONG_RANGE   //Time to read 1.3ms
...
void init_VL53L0X() //OCTOBER 2019 AS
{
         ...
         HAL_Delay(24);
					...
					// lower the return signal rate limit (default is 0.25 MCPS)
					 setSignalRateLimit(&sensor1,0.1);
				  #endif
...
//MOVE from the main file OCTOBER 2019
void I2C_Read_Byte(uint8_t addr,uint8_t *data)
{
		 while (HAL_I2C_GetState(&hi2c4) != HAL_I2C_STATE_READY);

		if(HAL_I2C_Master_Transmit(&hi2c4,  SLAVE_ADDRESS<<1 , &addr,  1, 100)!=HAL_OK)
			{
					_Error_Handler(__FILE__, __LINE__);
			}
		while (HAL_I2C_GetState(&hi2c4) != HAL_I2C_STATE_READY);
		if(HAL_I2C_Master_Receive(&hi2c4, ((SLAVE_ADDRESS <<1) | 0x01), data, 1, 100)!=HAL_OK)
			{
				 _Error_Handler(__FILE__, __LINE__);
			}
}
...
void I2C_Write_Byte(uint8_t addr, uint8_t data)
{
		...
		while (HAL_I2C_GetState(&hi2c4) != HAL_I2C_STATE_READY);
		if(HAL_I2C_Master_Transmit(&hi2c4, SLAVE_ADDRESS<<1, buf, 2, 100)!=HAL_OK)
			{
				_Error_Handler(__FILE__, __LINE__);
			}
}
